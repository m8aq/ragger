...
25. `fetch_category.py` — Stores whole wiki pages in `wiki_pages` as raw wikitext plus a markup-stripped `text` rendering. This is the only place page prose survives; every other script parses infoboxes into typed columns and discards the rest. By default it lists all non-redirect main-namespace articles via `list=allpages` (~83 requests, far cheaper than walking ~1,500 categories) and drops pages reachable only through non-content branches of the category graph: `Updates`, `Music`, `Discontinued content`, `Future content`, `Community`, `Jagex`, `Clans`, `Languages`, `Nonexistent content`, `Glitches`. `Content by release date` is deliberately neither walked nor used to drop anything — `Content released in 2006` tags 3,480 ordinary items and monsters. Because the graph is a DAG, exclusion is by reachability, not descendant walk: a category with any surviving path to `Content` after the excluded roots are removed is kept, which is what stops `Money making guides` from being dropped through `Economy`. `--category NAME` ingests just that category, stamping its name into `wiki_pages.source`; a full run stamps `Content` and replaces the whole table. Requires `fetch_categories.py` to have run.
